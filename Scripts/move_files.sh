...
function dot_files() {
  ...
  find ~/Public/dotfiles/ -type f -name ".*" | sed '3d'|sed '10d' | xargs md5sum | awk '{print $1 >> "dot_dir.txt"; print $2 >> "id_dot.txt"}'
...
function read_comp() {
  ...
  if [ "$check2" != "$check" ]; then
    grep -F $check2 id_root.txt | awk '{print "File "$2" Changed";print "Executing Copy";system("cp "$2" ~/Public/dotfiles/ ")}'
  fi
