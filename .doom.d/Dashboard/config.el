...
(defvar +doom-dashboard-menu-sections
  '(("Reload last session"
    ...
    ("New Roam Capture"
     ...
     :action org-roam-capture)
    ("List Bookmarks"
     :icon (all-the-icons-octicon "bookmark" :face 'doom-dashboard-menu-title)
     :action bookmark-bmenu-list))
...
(defun doom-dashboard-widget-footer ()
  (insert
   ...
   (+doom-dashboard--center
    ...
    (with-temp-buffer
      (insert-text-button (or (all-the-icons-fileicon "arch-linux" :face 'doom-dashboard-footer-icon :height 1.3 :v-adjust -0.15)
                              (propertize "Arch" 'face 'doom-dashboard-footer))
                          'action (lambda (_) (browse-url "https://github.com/hlissner/doom-emacs"))
                          'follow-link t
                          'help-echo "Welcome To Doom")
      (buffer-string)))
   "\n"))
